...
@router.get('/attempted-stages', response_model=List[schemas.AttemptedStageProtected])
def read_attempted_stages(skip: int = 0, limit: int = 100, db: Session = Depends(dependencies.get_db), logged_in_user: schemas.UserProtected = Depends(dependencies.get_logged_in_user)):
    attempted_stages = crud.attempted_stage.get_attempted_stages(
        db, logged_in_user, skip, limit)

    return attempted_stages

@router.post('/attempted-stages/create')
...
def read_best_attempted_stage(category: str, stage: int, db: Session = Depends(dependencies.get_db), logged_in_user: schemas.UserProtected = Depends(dependencies.get_logged_in_user)):
    attempted_stages = crud.attempted_stage.get_best_attempted_stage(
        db, logged_in_user, category, stage)
